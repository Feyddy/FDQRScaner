...
static NSTimeInterval FDQrLineanimateDuration = 0.02;


@implementation FDQRView{
...
- (void)layoutSubviews {
    ...
    if (!qrLine) {
        ...
        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:FDQrLineanimateDuration target:self selector:@selector(show) userInfo:nil repeats:YES];
        [timer fire];
    }
...
- (void)initQRLine {
    ...
    CGRect screenBounds = [FDQRUtil screenBounds];
...
- (void)initQrMenu {
    ...
    CGFloat height = 100;
    CGFloat width = [FDQRUtil screenBounds].size.width;
    qrMenu = [[FDQRMenuView alloc] initWithFrame:CGRectMake(0, [FDQRUtil screenBounds].size.height - height, width, height)];
    qrMenu.backgroundColor = [UIColor clearColor];
    [self addSubview:qrMenu];
    
    __weak typeof(self)weakSelf = self;
    
    qrMenu.didSelectedBlock = ^(FDQRButton *button){
        
        NSLog(@"点击的是%lu",(unsigned long)button.type);
        
        if ([weakSelf.delegate respondsToSelector:@selector(scanTypeConfig:)] ) {
            
            [weakSelf.delegate scanTypeConfig:button];
        }
    };
}
...
- (void)show {
    ...
    [UIView animateWithDuration:FDQrLineanimateDuration animations:^{
...
- (void)drawRect:(CGRect)rect {
    ...
    //整个二维码扫描界面的颜色
    CGSize screenSize =[FDQRUtil screenBounds].size;
